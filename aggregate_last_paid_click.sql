...
s.visitor_id, s.visit_date, l.created_at, l.status_id, amount, lead_id, closing_reason, medium, source, campaign 
from sessions s 
left join leads l 
...
tab as (select utm_source, utm_medium, utm_campaign, cast(campaign_date as date) as campaign_date,
...
group by 1,2,3,4),
 ...
 tab2 as
(select  
source, medium, campaign, cast(visit_date as date) as visit_date,
...
visitors_count, tab2.source as utm_source, tab2.medium as utm_medium, tab2.campaign as  utm_campaign,
total_cost,leads_count, purchases_count, revenue
from  tab2
left join tab
on tab2.medium = tab.utm_medium and 
tab2.source = tab.utm_source and 
tab2.campaign = tab.utm_campaign and 
tab2.visit_date = tab.campaign_date
